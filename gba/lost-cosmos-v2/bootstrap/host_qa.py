...
src=(root/'lost_cosmos_v2.c').read_text()
...
src=src.replace('static void wait_vblank(void){ while(REG_VCOUNT>=160){} while(REG_VCOUNT<160){} }',
...
src=src.replace('__attribute__((noinline)) void qa_done(void){ for(;;){} }',
...
src=src.replace('static void qa_fail(u32 code){ qa_stage=0xBAD00000u|code; qa_done(); }',
'''static void qa_fail(u32 code){ qa_stage=0xBAD00000u|code;
#ifdef HOST_QA
...
int main(int argc,char**argv){
 const char*save=(argc>1)?argv[1]:"hostqa.sav";
 ...
 printf("QA_STAGE=%08x SRAM126=%02x KEYS=%u WORLD=%u ENDING=%u POSTGAME=%u\\n",(unsigned)qa_stage,(unsigned)HOST_SRAM[126],(unsigned)keys_found,(unsigned)current_world,(unsigned)ending,(unsigned)postgame);
 return (qa_stage==0x51564131u||qa_stage==0x51564132u)?0:91;
}
#endif
'''
(root/'host_qa.c').write_text(src)
subprocess.run(['clang','-DQA_AUTORUN','-DHOST_QA','-O2','host_qa.c','-o','host_qa'],cwd=root,check=True)
save=root/'hostqa.sav'
if save.exists(): save.unlink()
for boot,expect in [(1,0xA5),(2,0x5A)]:
    p=subprocess.run([str(root/'host_qa'),str(save)],cwd=root,text=True,capture_output=True)
    ...
    got=data[126] if len(data)>126 else -1
...
print('PASS: same C gameplay QA completed two fresh-process boots with SRAM persistence')
