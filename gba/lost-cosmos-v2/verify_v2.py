...
rom=root/'SIM_EARTH_PIXEL_UNIVERSE_LOST_COSMOS_V2.gba'
...
if data[0xA0:0xAC] != b'LOSTCOSMOSV2': fail('title mismatch')
if data[0xAC:0xB0] != b'LCV2': fail('game code mismatch')
...
for token in ['MODE0','BG0_ENABLE','OBJ_ENABLE','MAP_W 64','generate_origin','generate_ember','generate_tide','generate_bloom','generate_black','generate_crown','GOAL_SEEK_KEY','GOAL_WANDER','MODEL != MEMORY','SRAM_V113','crown_interact','postgame_tick','QA_AUTORUN']:
    if token not in src: fail('source token missing: '+token)
# no undefined freestanding linker symbols
elf=root/'lost_cosmos_v2.elf'
if elf.exists():
...
ok('GBA header/logo/checksum, freestanding link, SRAM signature, exact QSEED and V2 engine surfaces verified')
