...
static void generate_surface(void){int i;REG_DISPCNT=0;for(i=0;i<MAP_W*MAP_H;i++){collision[i]=C_FREE;trigger[i]=TR_NONE;}set_world_palette(current_world);if(current_world==0 && current_room>=2 && current_room<=5){eridoria_area();}else if(current_room){generate_room();}else{if(current_world==6)generate_dreams();else if(current_world==7)generate_eldoria();else if(current_world==0)generate_origin();else if(current_world==1)generate_ember();else if(current_world==2)generate_tide();else if(current_world==3)generate_bloom();else if(current_world==4)generate_black();else generate_crown();if(current_world<6)generate_layer_overlay();}paint_v4_scenery();
 ...
 spawn_monsters();spawn_npcs();REG_BG0CNT=(u16)(2|(BG_TILE_CB<<2)|(BG_MAP_BASE<<8)|(3u<<14));REG_BG1CNT=(u16)((UI_TILE_CB<<2)|(UI_MAP_BASE<<8));ui_clear();REG_DISPCNT=MODE0|BG0_ENABLE|BG1_ENABLE|OBJ_ENABLE|OBJ_1D_MAP;}
...
static void draw_cinema_caption(void){
 static const char*HEAD[11]={"ERIDORIA // LOST COSMOS", "LUNA ARC // WARP", "ORIGIN EARTH", "EMBER AXIS", "TIDE MEMORY", "BLOOM Z", "BLACK GARDEN", "SYNAPSE CROWN", "DREAM VEIL", "ELDORIA", "MALAKAR // FIRST HEART"};
