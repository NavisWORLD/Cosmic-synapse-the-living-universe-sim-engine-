...
static void set_world_palette(int w){int i;u16 a=WORLDS[w].base0,b=WORLDS[w].base1,c=WORLDS[w].base2,d=WORLDS[w].accent;BG_PALETTE[0]=RGB5(0,0,0);for(i=0;i<16;i++){BG_PALETTE[i]=RGB5(0,0,0);}BG_PALETTE[1]=a;BG_PALETTE[2]=b;BG_PALETTE[3]=c;BG_PALETTE[4]=d;BG_PALETTE[5]=RGB5(27,27,31);BG_PALETTE[6]=RGB5(6,7,11);
 ...
 for(i=5;i<12;i++){OBJ_PALETTE[i*16+1]=RGB5(31,31,31);OBJ_PALETTE[i*16+2]=WORLDS[(i-5)%6].base2;OBJ_PALETTE[i*16+3]=WORLDS[(i-5)%6].accent;OBJ_PALETTE[i*16+4]=RGB5(31,8,8);OBJ_PALETTE[i*16+5]=RGB5(5,5,9);OBJ_PALETTE[i*16+6]=RGB5(31,28,16);}
 ...
 for(i=12;i<16;i++){OBJ_PALETTE[i*16+1]=RGB5(31,27,21);OBJ_PALETTE[i*16+2]=RGB5(3+i,8+i,20);OBJ_PALETTE[i*16+3]=WORLDS[(i-12)%6].accent;OBJ_PALETTE[i*16+4]=RGB5(25,9,15);OBJ_PALETTE[i*16+5]=RGB5(5,6,11);OBJ_PALETTE[i*16+6]=RGB5(31,29,20);}
}
...
static void music_step(void){if(!audio_on)return;music_tick++;if(music_tick>=36){u8 n=(u8)((frame/36)&7);music_tick=0;if(SONGS[game_mode==MODE_SPACE?5:WORLDS[current_world].music][n])tone((u16)(SONGS[game_mode==MODE_SPACE?5:WORLDS[current_world].music][n]+(frame&7)));}}
...
static void draw_intro(void){ui_clear();ui_fill_rows(2,17,63,15);ui_text(9,4,"SIM EARTH",14);ui_text(6,6,"PIXEL UNIVERSE",15);ui_text(7,8,"THE LOST COSMOS V5",13);ui_text(4,11,"THE UNIVERSE DID NOT DIE",15);ui_text(8,12,"IT LOST DEPTH",14);ui_text(4,15,"PRESS START TO WAKE",13);}
...
static void draw_pause(void){int i;ui_clear();ui_fill_rows(0,19,63,15);ui_text(2,1,"PAUSED // COSMIC RPG",14);
 if(pause_page==0){static const char*items[12]={"MAP","COSMOS","STATS","ITEMS","EQUIPMENT","MEMORIES","AXIS KEYS","STORY","SETTINGS","SAVE","QUESTS","NPC LOG"};for(i=0;i<12;i++){ui_text(2,3+i,(i==pause_sel)?">":" ",13);ui_text(4,3+i,items[i],(i==pause_sel)?14:15);}ui_text(2,16,"B+A HEAVY  B+SEL DODGE",14);ui_text(2,17,"A OPEN   B RESUME",13);}
...
static void update_surface(u16 k,u16 newk){int speed=(k&KEY_B)?3:2,dx=0,dy=0;u8 t;if(dodge_timer)dodge_timer--;if(dodge_cooldown)dodge_cooldown--;if(heavy_cooldown)heavy_cooldown--;if(npc_dialogue_active){
 if(newk&KEY_B)npc_close();else if(newk&KEY_A)npc_advance();return;
 }if(pending_choice){if(newk&KEY_LEFT){finalize_choice(1);return;}if(newk&KEY_UP){finalize_choice(2);return;}if(newk&KEY_RIGHT){finalize_choice(3);return;}return;}if(k&KEY_LEFT)dx=-speed;if(k&KEY_RIGHT)dx=speed;if(k&KEY_UP)dy=-speed;if(k&KEY_DOWN)dy=speed;if(dx&&dy){if((frame&1)==0)dy=0;else dx=0;}move_player(dx,dy,speed==3);if(player.hurt)player.hurt--;if(attack_timer)attack_timer--;if(magic_timer)magic_timer--;t=trigger_near();if(newk&KEY_A){int ni=npc_near();
