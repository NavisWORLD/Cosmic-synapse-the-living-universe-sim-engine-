...
#ifdef HOST_QA
static volatile u16 HOST_REG_DISPCNT,HOST_REG_BG0CNT,HOST_REG_BG1CNT,HOST_REG_BG2CNT,HOST_REG_BG0HOFS,HOST_REG_BG0VOFS,HOST_REG_BG1HOFS,HOST_REG_BG1VOFS,HOST_REG_BG2HOFS,HOST_REG_BG2VOFS,HOST_REG_VCOUNT,HOST_REG_KEYINPUT;
static volatile u16 HOST_REG_SOUNDCNT_L,HOST_REG_SOUNDCNT_H,HOST_REG_SOUNDCNT_X,HOST_REG_SOUND1CNT_L,HOST_REG_SOUND1CNT_H,HOST_REG_SOUND1CNT_X,HOST_REG_SOUND2CNT_L,HOST_REG_SOUND2CNT_H;
...
static void set_world_palette(int w){int i;u16 a=WORLDS[w].base0,b=WORLDS[w].base1,c=WORLDS[w].base2,d=WORLDS[w].accent;BG_PALETTE[0]=RGB5(0,0,0);for(i=0;i<16;i++){BG_PALETTE[i]=RGB5(0,0,0);}BG_PALETTE[1]=a;BG_PALETTE[2]=b;BG_PALETTE[3]=c;BG_PALETTE[4]=d;BG_PALETTE[5]=RGB5(27,27,31);BG_PALETTE[6]=RGB5(6,7,11);
 ...
 BG_PALETTE[13*16+1]=RGB5(31,25,8);BG_PALETTE[13*16+2]=RGB5(2,2,5);BG_PALETTE[14*16+1]=RGB5(8,31,29);BG_PALETTE[14*16+2]=RGB5(2,2,5);BG_PALETTE[15*16+1]=RGB5(31,31,31);BG_PALETTE[15*16+2]=RGB5(2,2,5);BG_PALETTE[13*16+3]=RGB5(31,25,8);BG_PALETTE[14*16+3]=RGB5(8,31,29);BG_PALETTE[15*16+3]=RGB5(5,19,26);
 /* OBJ palettes */
...
static void draw_cinema_caption(void){
 static const char*HEAD[8]={"ERIDORIA // LOST COSMOS", "LUNA ARC // WARP", "ORIGIN EARTH", "EMBER AXIS", "TIDE MEMORY", "BLOOM Z", "BLACK GARDEN", "SYNAPSE CROWN"};
...
static void draw_hud(void){char k[8];ui_clear();ui_fill_rows(0,1,63,15);ui_fill_rows(18,19,63,15);ui_text(1,0,game_mode==MODE_SPACE?"SPACE":location_name(),14);if(game_mode==MODE_SURFACE){ui_text(19,0,layer_name(),13);}
 ...
 if(npc_dialogue_active){const NPCDef*d=&NPCS[npc_dialogue_id];ui_frame(11,19,15);
 ...
 }else if(dialogue_timer){ui_frame(13,19,15);ui_text(2,14,"COSMOS",14);ui_wrap_text(15,dialogue,15,3);}else{ui_text(1,18,"A SWORD/ACT",14);ui_text(13,18,"B RUN",15);ui_text(20,18,"R MAGIC",13);ui_text(1,19,"L POTION",14);ui_text(8,19,"START MENU",15);ui_text(20,19,"Q",14);ui_num(22,19,qstate.phase,15);}
}
static void draw_battle(void){Enemy*e=&enemies[battle_index];
 ...
 oam_hide_all();oam_set(0,50,72,player.face*8,0,0);
 oam_set(1,72,76,32+(cosmos.mood&3)*4,1+(cosmos.mood&3),0);
 if(e->active)oam_set(2,172,58,60+e->type*4,5+e->type,0);
}
...
static const char* story_line(void){
 if(postgame)return "EPILOGUE // THE HEART ENDURES. ARIN AND COSMOS EXPLORE THE WORLDS STILL UNWRITTEN.";
...
static void draw_pause(void){int i;ui_clear();ui_fill_rows(0,19,63,15);ui_text(2,1,"PAUSED // COSMIC RPG",14);
 if(pause_page==0){static const char*items[12]={"MAP","COSMOS","STATS","ITEMS","EQUIPMENT","MEMORIES","AXIS KEYS","STORY","SETTINGS","SAVE","QUESTS","NPC LOG"};for(i=0;i<12;i++){ui_text(2,3+i,(i==pause_sel)?">":" ",13);ui_text(4,3+i,items[i],(i==pause_sel)?14:15);}ui_text(2,16,touch_mode?"SELECT DODGE   B+A HEAVY":"B+A HEAVY  B+SEL DODGE",14);ui_text(2,17,"A OPEN   B RESUME",13);}
...
static void load_game(void){int i;if(save_valid()||save_valid_v4()||save_valid_v3()){int old_v3=save_valid_v3(),old_v4=save_valid_v4();load_common();player_level=SRAM[128]?SRAM[128]:1;player_xp=(u16)sr16(129);max_hp=SRAM[131]?SRAM[131]:6;player_mp=SRAM[132];max_mp=SRAM[133]?SRAM[133]:6;str_stat=SRAM[134]?SRAM[134]:2;def_stat=SRAM[135]?SRAM[135]:1;mag_stat=SRAM[136]?SRAM[136]:2;credits=SRAM[137];for(i=0;i<ITEM_COUNT;i++)inv[i]=SRAM[138+i];gear_owned=SRAM[142]|1;weapon=SRAM[143];armor=SRAM[144];charm=SRAM[145];current_spell=SRAM[146]%SPELL_COUNT;buddy_talk=SRAM[147];buddy_quantum=SRAM[148];boss_flags=SRAM[149];kill_count=(u16)sr16(150);qstate.mean=SRAM[152];qstate.spread=SRAM[153];qstate.parity=SRAM[154];qstate.phase=SRAM[155];qstate.coherence=SRAM[156];qstate.burst=SRAM[157];if(player.hp>max_hp)player.hp=max_hp;
 ...
 else{quest_started=SRAM[158];quest_completed=SRAM[159];npc_seen=(u16)SRAM[160]|((u16)SRAM[161]<<8);npc_recent=SRAM[162];}workload_qi=(old_v3||old_v4)?qi:sr32(164);if(workload_qi>=QSEED_LEN)workload_qi=0;touch_mode=(!old_v3&&!old_v4&&((SRAM[168]&0xFE)==0xC0))?(SRAM[168]&1):0;if(old_v3||old_v4||((SRAM[168]&0xFE)!=0xC0))save_game();
...
static void battle_act(void){Enemy*e=&enemies[battle_index];int damage;
 ...
 if(battle_cursor==4){if(e->elite){battle_notice="THE GUARDIAN WON'T LET YOU FLEE.";}
  ...
  e->hurt=0;damage=str_stat+weapon_bonus()+(qstate.coherence>190&&buddy_quantum);
  ...
  player_mp-=2;e->hurt=0;damage_enemy(e,mag_stat+2+charm_bonus());battle_notice="COSMIC MAGIC FLOODS THE ARENA!";
...
static void update_battle(u16 newk){Enemy*e=&enemies[battle_index];
 ...
 if(battle_phase==1){
  ...
  if(!battle_timer){
   if(!battle_evade){
    int dmg=e->damage-(def_stat+armor_bonus())/4;if(dmg<1)dmg=1;
...
__attribute__((noinline)) void qa_done(void){
#ifdef HOST_QA
 return;
#else
 for(;;){}
...
static void qa_fail(u32 code){ qa_stage=0xBAD00000u|code; SRAM[126]=0xEE; SRAM[127]=(u8)code; save_game();
#ifdef HOST_QA
