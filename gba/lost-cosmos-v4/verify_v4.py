...
rom=root/'SIM_EARTH_PIXEL_UNIVERSE_LOST_COSMOS_V4.gba'
...
if data[0xA0:0xAC] != b'LOSTCOSMOSV4': fail('title mismatch')
if data[0xAC:0xB0] != b'LCV4': fail('game code mismatch')
if data[0xB2] != 0x96: fail('fixed header byte mismatch')
if data[0xBC] != 4: fail('software version mismatch')
...
ok('GBA header/logo/checksum, freestanding link, SRAM signature, exact QSEED and V4 RPG engine surfaces verified')
