...
typedef struct { s16 x,y; s8 vx,vy; u8 type,hp,maxhp,damage,active,hurt,elite; } Enemy;
...
static u8 attack_timer=0,magic_timer=0,workload_tick=0;
...
static u16 npc_seen=0; static u8 npc_recent=0;
...
static void quantum_workload_step(void){
 int i,sum=0,mn=255,mx=0,trans=0;u8 x=0,prev=next_q();
 for(i=0;i<8;i++){u8 v=(i==0)?prev:next_q();sum+=v;if(v<mn)mn=v;if(v>mx)mx=v;x^=v;if(i&&pop8((u8)(v^prev))>=4)trans++;prev=v;}
...
static void gen_player_frame(int base,int face,int step){
 u32 b[32];int i,x,y;for(i=0;i<32;i++)b[i]=0;
 /* 16x16 original pixel explorer with hair, gloves, moving boots, suit trim. */
 for(y=1;y<=2;y++)for(x=5;x<=10;x++)objpix(b,x,y,4);
 for(y=3;y<=6;y++)for(x=5;x<=10;x++)objpix(b,x,y,1);
 if(face!=0){objpix(b,6,4,3);if(face==1||face==3)objpix(b,9,4,3);}
 for(y=7;y<=11;y++)for(x=4;x<=11;x++)objpix(b,x,y,2);
 for(y=8;y<=9;y++)for(x=6;x<=9;x++)objpix(b,x,y,3); /* glowing 2px suit sigil */
 for(x=6;x<=9;x++)objpix(b,x,2,1); /* illustrated visor highlight */
 for(y=7;y<=10;y++){objpix(b,3,y,4);objpix(b,12,y,4);} /* sleeves */
 for(x=5;x<=10;x++)objpix(b,x,7,4); /* cosmic collar */
 if(armor){for(y=8;y<=11;y++){objpix(b,5,y,3);objpix(b,10,y,3);}for(x=6;x<=9;x++)objpix(b,x,11,3);}
 if(charm){objpix(b,7,9,1);objpix(b,8,9,3);}
 for(y=12;y<=14;y++){
  objpix(b,5+(step?1:0),y,2);objpix(b,6+(step?1:0),y,2);
  objpix(b,9-(step?1:0),y,2);objpix(b,10-(step?1:0),y,2);
 }
 objpix(b,5+(step?1:0),14,4);objpix(b,10-(step?1:0),14,4);
 if(weapon){int wx=face==2?1:13;for(y=6;y<=11;y++)objpix(b,wx,y,1);objpix(b,wx-1,10,3);objpix(b,wx+1,10,3);if(weapon==2){objpix(b,wx,5,3);objpix(b,wx,4,1);}}
 upload_obj16(base,b);
}
...
static void gen_enemy_frame(int base,int type){u32 b[32];int i,x,y;u8 body=(u8)(2+(type%5));for(i=0;i<32;i++)b[i]=0;for(y=3;y<13;y++)for(x=3;x<13;x++){int dx=x-8,dy=y-8;if(dx*dx+dy*dy<34)objpix(b,x,y,body);}objpix(b,5,7,1);objpix(b,10,7,1);if(type==EN_EMBER){for(x=4;x<12;x++)objpix(b,x,3,4);}if(type==EN_TIDE){for(x=3;x<13;x+=2)objpix(b,x,12,3);}if(type==EN_BLOOM){objpix(b,2,5,3);objpix(b,13,5,3);}if(type==EN_VOID){objpix(b,7,5,1);objpix(b,8,5,1);}if(type==EN_CROWN){for(x=4;x<12;x++)objpix(b,x,2,3);}upload_obj16(base,b);}
...
static void gen_npc_frame(int base,int kind,int frame_phase){u32 b[32];int x,y,i;for(i=0;i<32;i++)b[i]=0;
 for(y=3;y<=6;y++)for(x=5;x<=10;x++)objpix(b,x,y,1);
 for(y=1;y<=3;y++)for(x=4;x<=11;x++)if(y==1||kind==2||x==4||x==11)objpix(b,x,y,(kind&1)?4:2);
 objpix(b,6,4,5);objpix(b,9,4,5);objpix(b,7,6,4);objpix(b,8,6,4);
 for(y=7;y<=12;y++)for(x=4;x<=11;x++)objpix(b,x,y,(u8)(kind==3?4:2));
 for(y=8;y<=12;y++){objpix(b,3,y,kind==2?4:2);objpix(b,12,y,kind==0?3:2);}
 for(y=7;y<=11;y++)objpix(b,7,y,3);
 for(y=13;y<=15;y++){objpix(b,5+(frame_phase?1:0),y,5);objpix(b,10-(frame_phase?1:0),y,5);}
 if(kind==0){objpix(b,5,3,3);objpix(b,10,3,3);} /* archivist */
 if(kind==1){objpix(b,2,8,3);objpix(b,13,8,3);} /* wandering explorer */
 if(kind==2){for(x=5;x<=10;x++)objpix(b,x,2,4);} /* armored guardian */
 if(kind==3){for(x=6;x<=9;x++)objpix(b,x,10,1);} /* trader crest */
 upload_obj16(base,b);
}
...
static void make_obj_tiles(void){int f;refresh_player_frames();for(f=0;f<4;f++)gen_buddy_frame(32+f*4,f);gen_ship_frame(48);gen_planet_frame(52);gen_marker_frame(56);for(f=0;f<EN_COUNT;f++)gen_enemy_frame(60+f*4,f);gen_item_frame(84);gen_slash_frame(88);
 for(f=0;f<4;f++){gen_npc_frame(92+f*8,f,0);gen_npc_frame(96+f*8,f,1);} }
...
static void set_world_palette(int w){int i;u16 a=WORLDS[w].base0,b=WORLDS[w].base1,c=WORLDS[w].base2,d=WORLDS[w].accent;BG_PALETTE[0]=RGB5(0,0,0);for(i=0;i<16;i++){BG_PALETTE[i]=RGB5(0,0,0);}BG_PALETTE[1]=a;BG_PALETTE[2]=b;BG_PALETTE[3]=c;BG_PALETTE[4]=d;BG_PALETTE[5]=RGB5(27,27,31);BG_PALETTE[6]=RGB5(6,7,11);
 ...
 for(i=1;i<8;i++){BG_PALETTE[i*16]=RGB5(0,0,0);BG_PALETTE[i*16+1]=a;BG_PALETTE[i*16+2]=b;BG_PALETTE[i*16+3]=(i&1)?d:c;BG_PALETTE[i*16+4]=RGB5(4+i*2,5+i,9+i);BG_PALETTE[i*16+5]=RGB5(16+i,16+i,19+i);BG_PALETTE[i*16+6]=RGB5(27,27,31);BG_PALETTE[i*16+7]=RGB5(4,4,6);}
 ...
 for(i=5;i<12;i++){OBJ_PALETTE[i*16+1]=RGB5(31,31,31);OBJ_PALETTE[i*16+2]=WORLDS[(i-5)%6].base2;OBJ_PALETTE[i*16+3]=WORLDS[(i-5)%6].accent;OBJ_PALETTE[i*16+4]=RGB5(31,8,8);}
 ...
 for(i=12;i<16;i++){OBJ_PALETTE[i*16+1]=RGB5(31,27,21);OBJ_PALETTE[i*16+2]=RGB5(3+i,8+i,20);OBJ_PALETTE[i*16+3]=WORLDS[(i-12)%6].accent;OBJ_PALETTE[i*16+4]=RGB5(25,9,15);OBJ_PALETTE[i*16+5]=RGB5(5,6,11);}
}
...
static void npc_tick(void){int i,dx,dy;NPC*n;u8 q;
 ...
 for(i=0;i<npc_count;i++){n=&npc_runtime[i];if(!n->wanders)continue;
  q=(u8)(next_q()^qstate.parity^((u8)frame)+n->id*23);
...
static void generate_surface(void){int i;REG_DISPCNT=0;for(i=0;i<MAP_W*MAP_H;i++){collision[i]=C_FREE;trigger[i]=TR_NONE;}set_world_palette(current_world);if(current_room){generate_room();}else{if(current_world==0)generate_origin();else if(current_world==1)generate_ember();else if(current_world==2)generate_tide();else if(current_world==3)generate_bloom();else if(current_world==4)generate_black();else generate_crown();generate_layer_overlay();}paint_v4_scenery();spawn_monsters();spawn_npcs();REG_BG0CNT=(u16)(2|(BG_TILE_CB<<2)|(BG_MAP_BASE<<8)|(3u<<14));REG_BG1CNT=(u16)((UI_TILE_CB<<2)|(UI_MAP_BASE<<8));ui_clear();REG_DISPCNT=MODE0|BG0_ENABLE|BG1_ENABLE|OBJ_ENABLE|OBJ_1D_MAP;}
...
static void npc_speak(int rt){const NPCDef*d=&NPCS[npc_runtime[rt].id];
 ...
 npc_seen|=(u16)(1u<<npc_dialogue_id);npc_recent=npc_dialogue_id;save_game();
...
static const char* npc_line(void){const NPCDef*d=&NPCS[npc_dialogue_id];
 ...
 if(d->quest<6){int q=d->quest;if(quest_completed&(1u<<q))return d->after;
...
static void draw_hud(void){char k[8];ui_clear();ui_fill_rows(0,1,63,15);ui_fill_rows(18,19,63,15);ui_text(1,0,game_mode==MODE_SPACE?"SPACE":location_name(),14);if(game_mode==MODE_SURFACE){ui_text(19,0,layer_name(),13);}
 ...
 if(npc_dialogue_active){const NPCDef*d=&NPCS[npc_dialogue_id];ui_fill_rows(12,19,63,15);
 ui_text(3,12,d->name,14);ui_text(3,13,(d->quest<6&&(quest_completed&(1u<<d->quest)))?"MEMORY RECORDED":"THE WORLD IS SPEAKING",13);
 ui_wrap_text(14,npc_line(),15,4);ui_text(2,19,"A NEXT    B CLOSE",14);
 }else if(dialogue_timer){ui_fill_rows(15,17,63,15);ui_text(1,15,"COSMOS:",14);ui_wrap_text(16,dialogue,15,2);}else{ui_text(1,18,"A SWORD/ACT",14);ui_text(13,18,"B RUN",15);ui_text(20,18,"R MAGIC",13);ui_text(1,19,"L POTION",14);ui_text(8,19,"START MENU",15);ui_text(20,19,"Q",14);ui_num(22,19,qstate.phase,15);}
}
static void draw_intro(void){ui_clear();ui_fill_rows(2,17,63,15);ui_text(9,4,"SIM EARTH",14);ui_text(6,6,"PIXEL UNIVERSE",15);ui_text(7,8,"THE LOST COSMOS V4",13);ui_text(4,11,"THE UNIVERSE DID NOT DIE",15);ui_text(8,12,"IT LOST DEPTH",14);ui_text(4,15,"PRESS START TO WAKE",13);}
...
static void draw_pause(void){int i;ui_clear();ui_fill_rows(0,19,63,15);ui_text(2,1,"PAUSED // COSMIC RPG",14);
 if(pause_page==0){static const char*items[12]={"MAP","COSMOS","STATS","ITEMS","EQUIPMENT","MEMORIES","AXIS KEYS","STORY","SETTINGS","SAVE","QUESTS","NPC LOG"};for(i=0;i<12;i++){ui_text(2,3+i,(i==pause_sel)?">":" ",13);ui_text(4,3+i,items[i],(i==pause_sel)?14:15);}ui_text(2,17,"A OPEN   B RESUME",13);}
 else if(pause_page==1){ui_text(2,3,"MAP // KNOWN WORLDS",14);for(i=0;i<6;i++){ui_text(2,5+i,(visited_mask&(1u<<i))?"*":"-",13);ui_text(4,5+i,WORLDS[i].name,15);}ui_text(2,17,"B BACK",13);}
 ...
 else if(pause_page==3){ui_text(2,3,"PLAYER STATS",14);ui_text(2,5,"LEVEL",13);ui_num(10,5,player_level,15);ui_text(2,6,"XP",13);ui_num(10,6,player_xp,15);ui_text(2,7,"HP",13);ui_num(10,7,player.hp,15);ui_text(14,7,"/",15);ui_num(16,7,max_hp,15);ui_text(2,8,"MP",13);ui_num(10,8,player_mp,15);ui_text(14,8,"/",15);ui_num(16,8,max_mp,15);ui_text(2,10,"STR",13);ui_num(10,10,str_stat,15);ui_text(2,11,"DEF",13);ui_num(10,11,def_stat,15);ui_text(2,12,"MAG",13);ui_num(10,12,mag_stat,15);ui_text(2,13,"KILLS",13);ui_num(10,13,kill_count,15);ui_text(2,14,"CREDITS",13);ui_num(12,14,credits,15);ui_text(2,17,"B BACK",13);}
 else if(pause_page==4){ui_text(2,3,"ITEMS",14);for(i=0;i<ITEM_COUNT;i++){ui_text(2,5+i,(i==item_sel)?">":" ",13);ui_text(4,5+i,item_name(i),15);ui_num(22,5+i,inv[i],13);}ui_text(2,11,"UP/DOWN SELECT  A USE",14);ui_text(2,12,"L USES SELECTED ITEM",13);ui_text(2,17,"B BACK",13);}
 ...
 else if(pause_page==9){ui_text(2,3,"SETTINGS",14);ui_text(2,5,setting_sel==0?"> AUDIO":"  AUDIO",13);ui_text(16,5,audio_on?"ON":"OFF",15);ui_text(2,6,setting_sel==1?"> Q BUDDY":"  Q BUDDY",13);ui_text(16,6,buddy_quantum?"ON":"OFF",15);ui_text(2,7,setting_sel==2?"> AUTO TALK":"  AUTO TALK",13);ui_text(16,7,buddy_talk?"ON":"OFF",15);ui_text(2,10,"UP/DOWN SELECT A TOGGLE",14);ui_text(2,17,"B BACK",13);}
 else if(pause_page==10){ui_text(2,5,"SAVE COMPLETE",14);ui_text(2,7,"SRAM_V113 // V4",15);ui_text(2,9,"RPG + COSMOS STATE PERSISTED",13);ui_text(2,17,"B BACK",13);}
 else if(pause_page==11){static const char*QN[6]={"BEACON SHARD","FORGE TEST","OLD ARCHIVE","SEED MACHINE","LOST FUTURE","THREE AXES"};ui_text(2,3,"QUEST JOURNAL",14);
 for(i=0;i<6;i++){ui_text(2,5+i,quest_completed&(1u<<i)?"*":quest_started&(1u<<i)?"+":"-",13);ui_text(4,5+i,QN[i],15);}ui_text(2,13,"* DONE   + ACTIVE   - OPEN",13);ui_text(2,17,"B BACK",13);}
 else if(pause_page==12){int row=5;ui_text(2,3,"INHABITANTS MET",14);for(i=0;i<NPC_DEF_COUNT&&row<16;i++)if(npc_seen&(1u<<i)){
 ui_text(2,row,"*",13);ui_text(4,row++,NPCS[i].name,15);
...
static void spawn_enemy(int i,int tx,int ty,u8 type,u8 elite){Enemy*e=&enemies[i];e->x=(s16)(tx*8);e->y=(s16)(ty*8);e->vx=e->vy=0;e->type=type;e->elite=elite;e->maxhp=(u8)(2+type+(elite?5:0)+player_level/3);e->hp=e->maxhp;e->damage=(u8)(1+type/2+(elite?1:0));e->active=1;e->hurt=0;}
static void spawn_monsters(void){int i;clear_combat();if(current_room||game_mode!=MODE_SURFACE)return;for(i=0;i<5;i++){int tx=15+((i*11+current_world*7)%38),ty=14+((i*13+current_world*5)%30);if(collision[mi(tx,ty)]==C_WALL){tx=18+i*5;ty=34-i*3;}spawn_enemy(i,tx,ty,current_world,0);}if(current_world>=1&&current_world<=3&&!(boss_flags&(1u<<current_world))){int gx=current_world==1?49:(current_world==2?49:48),gy=current_world==1?12:(current_world==2?12:14);spawn_enemy(5,gx,gy,current_world,1);}if(current_world==4)spawn_enemy(6,27,18,EN_VOID,1);if(current_world==5)spawn_enemy(7,50,12,EN_CROWN,1);}
...
static void enemy_die(Enemy*e){u8 r;if(e->elite){boss_flags|=(u8)(1u<<current_world);drop_item_at(e->x,e->y,ITEM_CORE);credits=(u8)clampi(credits+12,0,255);say("ELITE SIGNAL COLLAPSED. COSMOS MARKED THE LOOT.");}else{r=(u8)((buddy_quantum?qstate.parity:next_q())+e->type*17);drop_item_at(e->x,e->y,(r%9==0)?ITEM_CORE:((r&1)?ITEM_POTION:ITEM_SHARD));credits=(u8)clampi(credits+1+e->type,0,255);}add_xp((u16)(4+e->type*2+(e->elite?15:0)));kill_count++;e->active=0;tone((u16)(500+e->type*90));}
static void damage_enemy(Enemy*e,int dmg){if(!e->active||e->hurt)return;if(dmg<1)dmg=1;e->hp=(u8)((dmg>=e->hp)?0:e->hp-dmg);e->hurt=10;if(!e->hp)enemy_die(e);}
static void player_attack(void){int i,ax=player.x,ay=player.y,dx=0,dy=0,range=25,dmg=str_stat+weapon_bonus();attack_timer=9;if(player.face==0)dy=-range;else if(player.face==1)dy=range;else if(player.face==2)dx=-range;else dx=range;ax+=dx;ay+=dy;for(i=0;i<10;i++)if(enemies[i].active&&iabs(enemies[i].x-ax)<22&&iabs(enemies[i].y-ay)<22)damage_enemy(&enemies[i],dmg+((buddy_quantum&&qstate.coherence>190)?1:0));tone((u16)(900+weapon*180));}
...
static void enemy_tick(void){int i;if(game_mode!=MODE_SURFACE||current_room)return;for(i=0;i<10;i++){Enemy*e=&enemies[i];int dx,dy,d;if(!e->active)continue;if(e->hurt)e->hurt--;d=iabs(e->x-player.x)+iabs(e->y-player.y);if(d<120&&(frame&3)==0){dx=signi(player.x-e->x);dy=signi(player.y-e->y);if(can_stand(e->x+dx,e->y))e->x+=dx;if(can_stand(e->x,e->y+dy))e->y+=dy;}if(d<14&&!player.hurt)hurt_player(signi(e->x-player.x),signi(e->y-player.y));}}
...
static void hurt_player(int dx,int dy){int dmg;if(player.hurt)return;player.hurt=40;dmg=1+(current_world/2);dmg-=((def_stat+armor_bonus())/5);if(dmg<1)dmg=1;player.hp=(u8)((dmg>=player.hp)?0:player.hp-dmg);player.x=(s16)clampi(player.x-dx*10,10,502);player.y=(s16)clampi(player.y-dy*10,10,502);tone(1000);cosmos.avoid=(u8)clampi(cosmos.avoid+10,0,255);if(player.hp==0){player.hp=max_hp;player_mp=max_mp;credits=(u8)(credits/2);player.x=80;player.y=400;say("I PULLED YOUR LAST STABLE POSITION FROM MEMORY. YOU LOST HALF YOUR CREDITS.");save_game();}}
...
static void world_light_tick(void){u16 a=WORLDS[current_world].accent;
 ...
 BG_PALETTE[3]=RGB5(r,g,b);BG_PALETTE[3*16+3]=RGB5(r,g,b);
}
...
static void gameplay_qa(void){
 ...
 if(SRAM[126]==0xA5 && save_valid()){
   qa_require((quest_started&1)!=0&&npc_seen,0x65);qa_require((keys_found&1)!=0,0x21); qa_require(current_world==1,0x22);
...
static void update_surface(u16 k,u16 newk){int speed=(k&KEY_B)?3:2,dx=0,dy=0;u8 t;if(npc_dialogue_active){
 if(newk&KEY_B)npc_close();else if(newk&KEY_A)npc_advance();return;
 }if(pending_choice){if(newk&KEY_LEFT){finalize_choice(1);return;}if(newk&KEY_UP){finalize_choice(2);return;}if(newk&KEY_RIGHT){finalize_choice(3);return;}return;}if(k&KEY_LEFT)dx=-speed;if(k&KEY_RIGHT)dx=speed;if(k&KEY_UP)dy=-speed;if(k&KEY_DOWN)dy=speed;if(dx&&dy){if((frame&1)==0)dy=0;else dx=0;}move_player(dx,dy,speed==3);if(player.hurt)player.hurt--;if(attack_timer)attack_timer--;if(magic_timer)magic_timer--;t=trigger_near();if(newk&KEY_A){int ni=npc_near();if(t)interact();else if(ni>=0)npc_speak(ni);else if(enemy_near_player(36))player_attack();else if(iabs(cosmos.x-player.x)<18&&iabs(cosmos.y-player.y)<18)buddy_speak_context();else player_attack();}if(newk&KEY_SELECT)drop_beacon();if(newk&KEY_L){if(t==TR_LIFT)shift_layer(-1);else use_potion();}if(newk&KEY_R){if(t==TR_LIFT)shift_layer(1);else cast_magic();}if(current_world==4&&current_room==0&&(frame&31)==0){int drift=(next_q()&1)?1:-1;if(!blocked_px(player.x+drift,player.y))player.x+=(s16)drift;}enemy_tick();collect_drops();npc_tick();buddy_tick();buddy_combat_assist();workload_buddy_tick();postgame_tick();}
...
static void update_pause(u16 newk){if(pause_page==0){if(newk&KEY_UP){if(pause_sel)pause_sel--;else pause_sel=11;}if(newk&KEY_DOWN){pause_sel=(u8)((pause_sel+1)%12);}if(newk&KEY_B){game_mode=return_mode;ui_clear();return;}if(newk&KEY_A){if(pause_sel<9)pause_page=(u8)(pause_sel+1);else if(pause_sel==9){save_game();pause_page=10;}else pause_page=(u8)(pause_sel+1);}}else{if(newk&KEY_B)pause_page=0;if(pause_page==4){if(newk&KEY_UP){if(item_sel)item_sel--;else item_sel=ITEM_COUNT-1;}if(newk&KEY_DOWN)item_sel=(u8)((item_sel+1)%ITEM_COUNT);if(newk&KEY_A)use_item(item_sel);}if(pause_page==5){if(newk&KEY_UP){if(equip_sel)equip_sel--;else equip_sel=3;}if(newk&KEY_DOWN)equip_sel=(u8)((equip_sel+1)&3);if(newk&KEY_A){if(equip_sel==0){if(weapon==0&&gear_owned&2)weapon=1;else if(weapon==1&&gear_owned&16)weapon=2;else weapon=0;}else if(equip_sel==1)armor=(u8)((gear_owned&4)?!armor:0);else if(equip_sel==2)charm=(u8)((gear_owned&8)?!charm:0);else{u8 n=current_spell;do{n=(u8)((n+1)%SPELL_COUNT);}while((n==SPELL_EMBER&&!(keys_found&1))||(n==SPELL_TIDE&&!(keys_found&2))||(n==SPELL_BLOOM&&!(keys_found&4)));current_spell=n;}refresh_player_frames();save_game();}}if(pause_page==9){if(newk&KEY_UP)setting_sel=(u8)((setting_sel+2)%3);if(newk&KEY_DOWN)setting_sel=(u8)((setting_sel+1)%3);if(newk&KEY_A){if(setting_sel==0)audio_on=(u8)!audio_on;else if(setting_sel==1)buddy_quantum=(u8)!buddy_quantum;else buddy_talk=(u8)!buddy_talk;save_game();}}}}
...
static void init_new_game(void){int i;quest_started=quest_completed=0;npc_seen=0;npc_recent=0;npc_dialogue_active=0;player.x=80;player.y=408;player.face=1;player_level=1;player_xp=0;max_hp=6;player.hp=max_hp;max_mp=6;player_mp=max_mp;str_stat=2;def_stat=1;mag_stat=2;credits=0;for(i=0;i<ITEM_COUNT;i++)inv[i]=0;inv[ITEM_POTION]=2;inv[ITEM_ETHER]=1;gear_owned=1;weapon=armor=charm=0;current_spell=SPELL_PULSE;buddy_talk=buddy_quantum=1;qstate.mean=128;qstate.coherence=128;qstate.phase=0;cosmos.x=100;cosmos.y=396;cosmos.goal=GOAL_FOLLOW;cosmos.trust=96;cosmos.curiosity=205;cosmos.avoid=30;cosmos.energy=240;cosmos.focus=130;current_world=0;current_room=0;current_layer=1;game_mode=MODE_SURFACE;ship_world=0;ship_x=PLANET_X[0];ship_y=PLANET_Y[0]+26;copystr(dialogue,"I REMEMBER A SKY MADE OF SQUARES.",90);}
