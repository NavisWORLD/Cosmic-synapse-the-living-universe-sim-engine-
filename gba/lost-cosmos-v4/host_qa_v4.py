...
src=(root/'lost_cosmos_v4.c').read_text()
...
int main(int argc,char**argv){
 const char*save=(argc>1)?argv[1]:"hostqa_v4.sav";
...
#endif
'''
(root/'host_qa_v4.c').write_text(src)
subprocess.run(['clang','-DQA_AUTORUN','-DHOST_QA','-O2','host_qa_v4.c','-o','host_qa_v4'],cwd=root,check=True)
save=root/'hostqa_v4.sav'
if save.exists(): save.unlink()
for boot,expect in [(1,0xA5),(2,0x5A)]:
    p=subprocess.run([str(root/'host_qa_v4'),str(save)],cwd=root,text=True,capture_output=True)
...
print('PASS: V4 RPG gameplay QA completed two fresh-process boots with SRAM persistence')
