...
void gba_main(void){u16 k,newk;init_new_game();load_game();init_graphics();sound_init();if(game_mode==MODE_SPACE)generate_space();else generate_surface();
 ...
 gameplay_qa();
#endif
for(;;){k=(u16)(~REG_KEYINPUT)&0x03FF;newk=(u16)(k&~prev_keys);prev_keys=k;if(intro){if(newk&KEY_START){intro=0;cinema_end();say("YOU WOKE ME WITH THE OLD TAPE. THREE AXIS KEYS ARE STILL BROADCASTING.");}}
