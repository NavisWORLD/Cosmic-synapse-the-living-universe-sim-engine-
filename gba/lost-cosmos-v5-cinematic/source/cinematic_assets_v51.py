...
W,H,SS=240,160,4
SCENES=('awakening','warp','origin','ember','tide','bloom','black_garden','synapse_crown')
...
def painting(sid):
 ...
 for y in range(H):
  if sid in (0,1,6,7):band=(y*3)//110
  else:band=(y*3)//98
  col=p[min(4,1+band)]
...
def main():
 ...
 hdr=['/* Generated with cinematic_assets_v51.py: original 960x640 paintings reduced into real GBA 4bpp scenes. */',
      ...
      '#define V51_SCENE_COUNT 8',
      'static const u16 V51_SCENE_OFFSETS[8]={'+','.join(str(z) for z in offsets)+'};',
      'static const u16 V51_SCENE_COUNTS[8]={'+','.join(str(z) for z in counts)+'};',
      'static const u16 V51_SCENE_COLORS[8][16]={'+','.join('{'+','.join(f'0x{z:04x}' for z in pal)+'}' for pal in palettes)+'};',
      'static const u16 V51_SCENE_MAPS[8][600]={']
