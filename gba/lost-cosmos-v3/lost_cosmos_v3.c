...
static void gen_player_frame(int base,int face,int step){
 ...
 if(face!=0){objpix(b,face==2?6:6,4,3);if(face==1||face==3)objpix(b,9,4,3);}
...
static void generate_surface(void){int i;REG_DISPCNT=0;for(i=0;i<MAP_W*MAP_H;i++){collision[i]=C_FREE;trigger[i]=TR_NONE;}set_world_palette(current_world);if(current_room){generate_room();}else{if(current_world==0)generate_origin();else if(current_world==1)generate_ember();else if(current_world==2)generate_tide();else if(current_world==3)generate_bloom();else if(current_world==4)generate_black();else generate_crown();generate_layer_overlay();}spawn_monsters();REG_BG0CNT=(u16)(2|(BG_TILE_CB<<2)|(BG_MAP_BASE<<8)|(3u<<14));REG_BG1CNT=(u16)((UI_TILE_CB<<2)|(UI_MAP_BASE<<8));ui_clear();REG_DISPCNT=MODE0|BG0_ENABLE|BG1_ENABLE|OBJ_ENABLE|OBJ_1D_MAP;}
...
static void draw_hud(void){char k[8];ui_clear();ui_fill_rows(0,1,63,15);ui_fill_rows(18,19,63,15);ui_text(1,0,game_mode==MODE_SPACE?"SPACE":location_name(),14);if(game_mode==MODE_SURFACE){ui_text(19,0,layer_name(),13);}
 ...
 if(dialogue_timer){ui_fill_rows(15,17,63,15);ui_text(1,15,"COSMOS:",14);ui_wrap_text(16,dialogue,15,2);}else{ui_text(1,18,"A SWORD/ACT",14);ui_text(13,18,"B RUN",15);ui_text(20,18,"R MAGIC",13);ui_text(1,19,"L POTION",14);ui_text(8,19,"START MENU",15);ui_text(20,19,"Q",14);ui_num(22,19,qstate.phase,15);}
}
static void draw_intro(void){ui_clear();ui_fill_rows(2,17,63,15);ui_text(9,4,"SIM EARTH",14);ui_text(6,6,"PIXEL UNIVERSE",15);ui_text(7,8,"THE LOST COSMOS V3",13);ui_text(4,11,"THE UNIVERSE DID NOT DIE",15);ui_text(8,12,"IT LOST DEPTH",14);ui_text(4,15,"PRESS START TO WAKE",13);}
...
static void draw_pause(void){int i;ui_clear();ui_fill_rows(0,19,63,15);ui_text(2,1,"PAUSED // COSMIC RPG",14);
 if(pause_page==0){static const char*items[10]={"MAP","COSMOS","STATS","ITEMS","EQUIPMENT","MEMORIES","AXIS KEYS","STORY","SETTINGS","SAVE"};for(i=0;i<10;i++){ui_text(2,3+i,(i==pause_sel)?">":" ",13);ui_text(4,3+i,items[i],(i==pause_sel)?14:15);}ui_text(2,17,"A OPEN   B RESUME",13);}
 ...
 else if(pause_page==10){ui_text(2,5,"SAVE COMPLETE",14);ui_text(2,7,"SRAM_V113 // V3",15);ui_text(2,9,"RPG + COSMOS STATE PERSISTED",13);ui_text(2,17,"B BACK",13);}
}
...
static void load_game(void){int i;if(save_valid()){load_common();player_level=SRAM[128]?SRAM[128]:1;player_xp=(u16)sr16(129);max_hp=SRAM[131]?SRAM[131]:6;player_mp=SRAM[132];max_mp=SRAM[133]?SRAM[133]:6;str_stat=SRAM[134]?SRAM[134]:2;def_stat=SRAM[135]?SRAM[135]:1;mag_stat=SRAM[136]?SRAM[136]:2;credits=SRAM[137];for(i=0;i<ITEM_COUNT;i++)inv[i]=SRAM[138+i];gear_owned=SRAM[142]|1;weapon=SRAM[143];armor=SRAM[144];charm=SRAM[145];current_spell=SRAM[146]%SPELL_COUNT;buddy_talk=SRAM[147];buddy_quantum=SRAM[148];boss_flags=SRAM[149];kill_count=(u16)sr16(150);qstate.mean=SRAM[152];qstate.spread=SRAM[153];qstate.parity=SRAM[154];qstate.phase=SRAM[155];qstate.coherence=SRAM[156];qstate.burst=SRAM[157];if(player.hp>max_hp)player.hp=max_hp;return;}if(save_valid_v2()){load_common();player_level=1;player_xp=0;max_hp=6;player.hp=clampi(player.hp?player.hp:3,1,max_hp);player_mp=max_mp=6;str_stat=2;def_stat=1;mag_stat=2;inv[ITEM_POTION]=2;inv[ITEM_ETHER]=1;gear_owned=1;weapon=armor=charm=0;buddy_talk=buddy_quantum=1;save_game();}}
...
static void update_surface(u16 k,u16 newk){int speed=(k&KEY_B)?3:2,dx=0,dy=0;u8 t;if(pending_choice){if(newk&KEY_LEFT){finalize_choice(1);return;}if(newk&KEY_UP){finalize_choice(2);return;}if(newk&KEY_RIGHT){finalize_choice(3);return;}return;}if(k&KEY_LEFT)dx=-speed;if(k&KEY_RIGHT)dx=speed;if(k&KEY_UP)dy=-speed;if(k&KEY_DOWN)dy=speed;if(dx&&dy){if((frame&1)==0)dy=0;else dx=0;}move_player(dx,dy,speed==3);if(player.hurt)player.hurt--;if(attack_timer)attack_timer--;if(magic_timer)magic_timer--;t=trigger_near();if(newk&KEY_A){if(t)interact();else if(!enemy_near_player(48)&&iabs(cosmos.x-player.x)<18&&iabs(cosmos.y-player.y)<18)buddy_speak_context();else player_attack();}if(newk&KEY_SELECT)drop_beacon();if(newk&KEY_L){if(t==TR_LIFT)shift_layer(-1);else use_potion();}if(newk&KEY_R){if(t==TR_LIFT)shift_layer(1);else cast_magic();}if(current_world==4&&current_room==0&&(frame&31)==0){int drift=(next_q()&1)?1:-1;if(!blocked_px(player.x+drift,player.y))player.x+=(s16)drift;}enemy_tick();collect_drops();buddy_tick();buddy_combat_assist();workload_buddy_tick();postgame_tick();}
